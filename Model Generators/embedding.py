...
for index in range(1, 201):
    file = open(doc_path + '/Document_' + str(index) + '.txt', 'r', encoding='utf-8')
    texts = file.readlines()
    file.close()
    texts = pre_process(texts)
    for line in texts:
        globalCorpus.append(line)
documents = [TaggedDocument(doc, [i]) for i, doc in enumerate(globalCorpus)]
print(documents)
model = Doc2Vec(documents, vector_size=10, window=5, min_count=1, min_alpha=.01, workers=4)
